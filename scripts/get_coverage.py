...
os.symlink('data/final_long.sort.bam', 'data/binning_bams/p0.sort.bam')
os.symlink('data/final_long.sort.bam.bai', 'data/binning_bams/p0.sort.bam.bai')
bam_list.append('data/binning_bams/p0.sort.bam')
set_num = 1
if snakemake.config['short_reads_2'] != 'none':
    already_done.add((snakemake.config['short_reads_1'], snakemake.config['short_reads_2']))
    os.symlink('data/final_short.sort.bam', 'data/binning_bams/p1.sort.bam')
    os.symlink('data/final_short.sort.bam.bai', 'data/binning_bams/p1.sort.bam.bai')
    bam_list.append('data/binning_bams/p1.sort.bam')
    set_num += 1
        ...
        for line in f:
            if line.split()[0] == 'nanopore':
                ...
                if not os.path.exists(r):
                    ...
                    bam_list.add("data/binning_bams/p%d.sort.bam" % set_num)
                    set_num += 1
...
with open("data/metabat.cov") as f, open("data/maxbin.cov.list", "w") as o:
    ...
    for i in range(len(depths[0])):
        with open("data/maxbin_cov/p%d.cov" % i, 'w'):
            for j in range(len(contig_list)):
                o.write(contig_list[j] + '\t' + cov_list[j][i] + '\n')
